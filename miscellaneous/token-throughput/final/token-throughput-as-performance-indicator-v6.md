...
*Why your AI usage bill is a competitive signal, not a line item to shrink. Token spend should be high — the cost should stay reasonable. Excluding, naturally, directly wasteful loops, goals and agentic setups.*
...
Before going further, one correction to the ambient conversation. The public debate about AI at work has narrowed almost entirely onto software development — copilots, vibe coding, autonomous agents shipping pull requests. That is a real and important domain. It is also, at most, a tenth of where an organization's Token Spenditure should be going.

The narrowing is understandable: developers were early adopters, the output is easy to count, and the vendors sell to them. But an organization that reads "AI intensity" as "our engineers use Copilot" has confined the most significant operational shift in a generation to a single department. The question isn't how much AI your developers use. It's how much of your organization's *thinking* has moved.

Work that consumes tokens at real volume, none of it code:

- **Legal and contract operations** — clause extraction across a contract portfolio, obligation tracking, comparing supplier terms against a standard, flagging renewal risk continuously rather than at renewal.
- **Regulatory and compliance monitoring** — agents watching regulatory change across every jurisdiction you operate in, mapping each change to the specific internal policies and systems it touches.
- **Procurement and tendering** — reading every RFP response in full, comparing against requirements line by line, drafting and stress-testing bids before submission.
- **Finance and audit** — reconciliation, anomaly detection across transaction volume no human samples, scenario modelling with hundreds of variants rather than three.
- **Research and R&D synthesis** — literature review that actually reads the literature, patent landscape mapping, competitive technical analysis maintained continuously instead of quarterly.
- **Engineering, quality and configuration testing** — failure-mode analysis, root-cause investigation across service history, specification-compliance checking, and systematic configuration testing in modular systems: change one component, interface or parameter and automatically surface what breaks, drifts or degrades in the rest of the system. The work that today is done by sparse manual regression, or by hoping the next integration test catches it.
- **Data and master data management** — entity resolution, deduplication, taxonomy maintenance, and the unglamorous data-quality work that quietly blocks every downstream AI initiative.
- **Design and user experience** — design system audits across every component and state, accessibility sweeps at full-product scale, generating and evaluating interface copy variants, heuristic evaluation of flows nobody has time to test.
- **Customer operations** — not just deflecting support tickets, but reading every ticket ever filed to find the product defect underneath the pattern.
- **Security operations** — log analysis, threat hunting, incident triage, and the continuous review of configuration drift that no team is staffed to do by hand.
- **Documentation and institutional knowledge** — keeping documentation true as systems change, rather than letting it rot between rewrites.
- **Media and content production** — generating, evaluating, and iterating on video, audio, imagery, and copy at volumes that make traditional production economics irrelevant.
- **Planning and forecasting** — demand planning, capacity modelling, and supply scenarios explored in parallel rather than picked from a shortlist someone had time to build.
- **Strategy and decision support** — red-teaming your own plan, war-gaming a competitor response, arguing both sides of an investment case before the meeting rather than during it.

Two things stand out about this list. First, most of these are functions where the organization currently does a *sampled* version of the work — auditing some contracts, reviewing some tickets, testing some flows — because doing it exhaustively was never affordable. Agentic capacity turns sampling into coverage, and coverage is where the value hides.

Second, almost none of it appears in the AI discourse. That is the opportunity. If your competitors are all optimizing the same coding workflow, the advantage is in the thirteen other rows.
...
## Exploration Comes Before Structure

There is a tension in this paper worth naming before it becomes an objection. Everything above argues for structure — proxies, dashboards, architectural decoupling, Value measured against outcomes. But you cannot design that structure in advance. The patterns worth institutionalizing are not knowable from a planning session; they are discovered by people using the tools on their own work, in ways nobody specified.

This matters because the instinct in most organizations is the reverse: define the approved use cases, write the policy, roll out training, then measure compliance. Applied to agentic tooling, that sequence produces a narrow, safe, and largely worthless deployment — because the interesting uses are precisely the ones nobody would have thought to authorize. The lawyer who starts having agents read every contract rather than a sample. The designer who runs an accessibility sweep across a product nobody had budget to test. The planner who explores forty supply scenarios instead of three. None of those appear on a use-case list written by a steering committee.

So the honest sequence is: **explore first, structure second.** Give people real access, real budget, and explicit permission to use it in whatever way they find useful for their own work — including ways that turn out to be dead ends. Exploratory Spenditure looks wasteful on a dashboard, because its Value attribution is weak by definition: you don't yet know what you're looking for. Killing it early to protect the Cost line is the same mistake as throttling usage generally, only harder to spot, because it wears the costume of prudence.

The cheapest instrument for turning that exploration into structure is time to talk. Dedicated, recurring meeting time where people share what they tried, what worked, what quietly failed, and which prompt or agent arrangement turned a two-day task into twenty minutes. Not a governance forum and not a training session — a peer exchange, where the tips and tricks that would otherwise stay in one person's head become organizational capability. Most of what an organization eventually writes into its AI standards will have been discovered by one person and shared in a room like that.

This also answers a question the three needles cannot answer on their own. Spenditure, Cost, and Value tell you where you are. They don't tell you how to get somewhere better. The mechanism for that is people trying things and telling each other what happened — and an organization that funds the tokens but not the conversation has bought the expensive half.

## Goals Cost More Than Specifications

There is a second reason Spenditure climbs, and it is structural rather than behavioural.

A detailed specification is cheap to execute because the expensive part already happened. Someone narrowed the problem, discarded the alternatives, and handed over an implementation-ready description. That work took meetings, weeks, and salaried hours — but it never appeared on any compute line, because it happened in people's heads.

Open goals invert that. When you state an outcome and its constraints rather than a solution, you are asking the system to do the searching: generate candidates, evaluate them against the goal, discard most, refine the rest, and try again. That search is the expensive part, and it is now denominated in tokens rather than in calendar time.

So the cost did not appear. It moved — from invisible human deliberation to a visible invoice. This is the single most misread line in any AI budget. A team that shifts from specifying solutions to setting goals will see its token consumption rise sharply, and will be tempted to read that as inefficiency. It is closer to the opposite: it means the organization has stopped pre-shrinking problems down to what a person could specify in a document before work began.

That pre-shrinking is worth naming, because it is so normal it is invisible. Requirements get trimmed to fit the capacity assumed available. Ambition is negotiated down at every handover — from idea to brief, from brief to backlog, from backlog to what actually ships. Each step is individually reasonable and collectively expensive. When execution capacity was genuinely scarce, that trimming was prudence. When it isn't, the same habit quietly caps the organization's ceiling at whatever one person could imagine being able to ask for.

Which is where the token bill becomes a measure of ambition rather than of waste. Muhammad Ali's line about impossible being an opinion rather than a fact is usually read as motivational. Read it operationally instead: a great many things filed under *impossible* were only ever unaffordable to search for. The search is what just got cheap.
...
|---|---|---|
| **Primary goal** | Minimize the AI usage bill | Maximize AI integration density |
...
Three things this short paper deliberately leaves open, each large enough to deserve its own: how to attribute Token Value to outcomes when effects are lagged and multi-causal; what has to change in review loops, approval rights, and trust calibration before high Spenditure is safe at scale — Section 3 argues for exploration and shared learning, but that is a starting posture, not a governance model; and how organizations that can't build private clusters achieve the same decoupling through open models, efficient fine-tunes, and shared infrastructure. Naming them is not the same as solving them — but a framework that pretended they were solved would be worth less than one that doesn't.
