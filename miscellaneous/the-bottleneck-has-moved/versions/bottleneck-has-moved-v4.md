...
*Draft v4 — working draft, not a release candidate. Revised after six independent review rounds.*
...
*End of Draft v4 — working draft, not a release candidate.*

*Changes from v3: Section 6 rebuilt in established portfolio-management vocabulary — "retrospective auction" and "bidding" replaced throughout by the Retrospective Portfolio Showcase and adoption decisions, so the mechanism reads as an adjustment to machinery organizations already run rather than an import from a different discipline; adoption decisions specified concretely (shared-resource priority, rollout funding, official portfolio adoption, expanded agent/compute capacity); contended-resource arbitration added with a timebox and escalation path; two-track funding rendered as a table; Track A (competitive) and Track B (collegial allocation) added as cultural variants; anti-pattern note on presentation-over-substance; Section X restated as a precondition rather than an appendix. Compute and token cost added alongside human cognitive bandwidth as the two constraints bounding span of accountability (Sec 5). Controlled Duplication replaces Repetitive Work in Section X, drawing a sharp line between free duplication in PoCs and strict architectural governance of core systems and shared domain logic, with an integration path back into core under PO-group oversight. "No new hire" qualified honestly in Section 9.*

*Open items for v5 — genuinely unresolved, not deferred content: (1) concrete example-level mechanisms for architectural consistency and shared contracts — flagged by five of six reviewers as the largest remaining gap, and now load-bearing for Section 6 as well; (2) sharper statement of domains where this model fits poorly — core banking, telecom, deeply coupled legacy platforms, heavily regulated environments — currently implicit rather than stated; (3) deeper treatment of isolation and social capital, since guilds and PO round-tables are the first thing to lapse under pressure and currently get one sentence; (4) a concrete model for auditing outcome accountability without producing personal fear.*
