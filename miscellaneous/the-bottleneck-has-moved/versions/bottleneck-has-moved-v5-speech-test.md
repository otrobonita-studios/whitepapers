...
Draft v5 — working draft, not a release candidate.
...
That does not make the model inapplicable in these organizations, but it does change where to apply it. The plugin fits best at the edges — new products, customer-facing surfaces, internal tooling, data and analytics work, greenfield services — and fits worst at the regulated, tightly-coupled core. A bank might reasonably run twenty agentic teams across digital channels and internal platforms while leaving the ledger under its existing structure. That is not a partial adoption failure; it is correct application.
...
What an Adoption Decision Actually Allocates

"Adoption" is not an abstract endorsement, and it is not primarily about money. A decision may allocate one or more of:
...
When two or more deliveries compete for the same constrained resource — a shared data platform, a single security review slot, limited compliance capacity — the conflict is resolved by the Portfolio Curator together with the affected Product Owners, with strategic themes as the deciding criterion. If no agreement is reached within a defined time window, the decision escalates to whoever owns those strategic themes. The goal is fast clarity, not consensus at any cost.

Cultural Variant: Competitive or Collegial

The inverted logic does not require internal rivalry. Two modes are equally valid, and the choice is cultural rather than structural:
...
More fundamentally: without working mechanisms for architectural alignment and shared contracts, this model rewards locally optimal but globally inconsistent solutions. Section X is therefore not an appendix — it is a precondition for adoption decisions to create value rather than technical debt.
...
If you already have a great team setup — dynamic co-work, few earbuds, genuine fun — this plugin might not be for you. It's built for organizations where that dynamic has already eroded, or never existed at scale, not to replace one that's working.
...
There's no universally correct recipe — only a correct one for your organization's risk appetite and how visible your competitors' speed already is to your board. What all five share: none require expanding the development organization, and all are reversible in their early phases.

One honest qualification. "No new hires" is close to true for delivery capacity, but not a blanket promise. The three load-bearing functions in Section 5 — Curation, Routing, Governance — plus standing legal and compliance access will in practice require redeploying senior capability, and in some organizations adding it. That is a small investment against the capacity change, but it should be planned for rather than discovered.
...
Phases: pilot team splits voluntarily → measure → run one Retrospective Portfolio Showcase with real adoption decisions → scale by pull as numbers become visible.
...
Franchise
One business unit or product line goes fully agentic — POs, showcase cycle, adoption decisions, the works — while everyone else watches. Once proven, the pattern, not the people, gets replicated unit by unit.
...
End of Draft v5 — working draft, not a release candidate.

Changes from v4: new subsection "Where This Model Is Weakest" added to Section 4 — stating plainly that the model degrades where integration density, regulatory coupling, or legacy entanglement is high (core banking, telecom network functions, safety-critical and certified systems, deeply shared domain models), explaining why the coordination load multiplies rather than reduces in those environments, and framing edge-first adoption in such organizations as correct application rather than partial failure.

Changes in v4: Section 6 rebuilt in established portfolio-management vocabulary — "retrospective auction" and "bidding" replaced throughout by the Retrospective Portfolio Showcase and adoption decisions; adoption decisions specified concretely (shared-resource priority, rollout funding, official portfolio adoption, expanded agent/compute capacity); contended-resource arbitration with timebox and escalation; two-track funding rendered as a table; Track A (competitive) and Track B (collegial allocation) as cultural variants; anti-pattern note on presentation-over-substance; Section X restated as precondition rather than appendix. Compute and token cost added alongside human cognitive bandwidth as the two constraints bounding span of accountability (Sec 5). Controlled Duplication replaces Repetitive Work in Section X. "No new hire" qualified honestly in Section 9. Social isolation treated substantially in Section X — coordination rituals framed as lifted to the PO layer and showcase cycle rather than removed, with touchpoint design handed to management as an explicit responsibility rather than prescribed as ceremonies.

Open items for v6 — genuinely unresolved: (1) concrete example-level mechanisms for architectural consistency and shared contracts — flagged by every reviewer as the largest remaining gap, now load-bearing for Section 6 as well; needs real design work on who owns shared contracts, how a Product Owner discovers they are about to violate one, and what happens when a delivery requires changing a shared schema; (2) a concrete model for auditing outcome accountability without producing personal fear — Section 7 states the principle but offers no mechanism; (3) Section X should likely be restructured and renamed ("Open Design Decisions") before any release candidate, since it reads as a set of named questions rather than a resolved section.
