...
*Draft v3 — revised after second external review round (four independent reviews total)*
...
Two functions, however, are non-negotiable in some form, because the rest of the model depends on them: **verification** (the Reviewer and Specification QA functions below, however named) and a **market-ready record** (the Chronicler function) — without the first, accountability is nominal; without the second, the retrospective market in Section 6 has nothing to evaluate.
...
*Changes from v2: new Section 3.5 "The Psychological Bureaucracy Trap" — counting the lone human is the model's most common misreading, one that infected early drafts of this paper; the three support functions (Curator, Router, Governance Advisor) reframed as agentic teams and pull-based enabling services, not a human coordination layer (Sec 5); agent cast made explicitly illustrative rather than prescriptive — composition, naming, and size are the PO's call, bounded only by their comfortable span of accountability, with verification and a market-ready record as the two non-negotiable functions (Sec 5); colophon added.*

*Open items for v4, consolidated from all four reviews: concrete (example-level) mechanisms for architectural consistency and shared contracts; auction bidding mechanics (what a bid consists of; conflict resolution over shared resources); token/compute economy alongside human-in-the-loop cost in the budget argument; duplication boundary (free at the edges, locked for core domain logic); optional "Track B" collegial-allocation variant of the retrospective market for consensus-driven cultures; honest headcount note on the load-bearing roles; sharper statement of domains where the model fits poorly (high integration, heavy regulation, deep legacy).*
