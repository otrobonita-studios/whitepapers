...
*Why organizations quietly shrink requirements before they are written — and where the cost actually lands.*

*An upstream practice note for organizations running SAFe, Scrum, LeSS or Kanban.*
...
A UX designer sits down to contribute to a requirement. After thorough user research, stakeholder interviews, and weeks of synthesis, he believes he knows exactly what the product needs. He also knows the team that will build it — he has read their skill composition, their history, their scar tissue — and he knows they cannot deliver it. So he contributes half of what he knows.

Nobody told him to do this. Nobody will ever know he did it. The requirement that reaches the backlog looks complete, reasonable, well-formed. It will be estimated, built, and shipped, and the delivery metrics will record a success. What the metrics cannot record is that the ask was discounted before it was ever voiced.

The designer is a composite. The behavior is not — it is drawn from many such moments across three decades of practice, including my own.
...
Not every organization discounts. There are environments — the space industry, motorsport, offshore yachting, haute couture, luxury automotive — where the half-ask was never viable. When the requirement is a lap time set by physics, a garment worn by one person on one night, or a system that must function in orbit, the gap between what is needed and what is asked closes to zero by necessity. The cost of the discount is immediate and visible. These industries did not develop the habit this paper describes because they could never afford to.

Naturally, a small number of prestige projects in other industries also operate at full price — certain flagship products, high-stakes platforms, or executive-sponsored initiatives where the cost of the half-ask is politically or commercially intolerable. If you work on one of these, you are the exception. Most organizations are not so fortunate. This paper is written for the far more common case: the environments where the discount remains invisible, the metrics stay clean, and the innovation that was never requested leaves no trace.
...
## The Literature That Institutionalized the Discount
...
*Figure 1 — The two paths from a single need, and where the cost lands.*

The discount is not a saving. It is a transfer.

[Figure 1: the two paths from a single need — the discount path and the full-price path — and where the transferred cost lands.]

When a requirement is written at half price, the cost of the missing half does not evaporate. It moves — out of the development estimate, where it would have been visible, owned, and argued about, and into parts of the organization that never sat in the requirements conversation. The delivery team ships on time and on budget. Somebody else pays.

To customer support. The onboarding flow that could have been two minutes stays at twenty. Every new user now needs guiding through it, formally or by trial and error. Tickets accumulate around "how do I…" The support function absorbs the cost of what was never built. One requirement, discounted once, becomes years of support load — and nobody computes the total, because support headcount and feature estimates live in different budgets.

To operations. A workflow that could have been fully automated ships at half automation, requiring manual intervention at three decision points. Operations manages the exception handling, documents the manual steps, trains every new hire on them. The discount converted "build it right" into "build it and we will manage it" — permanently, in headcount.

To everyone's working day. This is the most expensive form and the least visible. A mandated internal system — time reporting, expense approval, access requests, anything the business rules require people to visit regularly — is built with the awkward parts left out, because trimming them looked like a sensible saving during development. Now it takes ninety seconds instead of thirty, or four clicks instead of one. Multiply by the whole organization, every week, for the life of the system. Thousands of hours a year, drawn from every function simultaneously, appearing on no line item and attributed to no decision. A system that everybody is required to use is the single worst place in an organization to take a discount, and it is routinely where the discount is taken first — precisely because the users are captive and cannot churn.

To every team downstream of a foundation. A design system is mandated across the portfolio and ships without full component coverage, without interaction states specified, without accessibility built in. It cannot be avoided, and it cannot do what teams need. So every team extends it, works around it, or quietly rebuilds it. The artifact intended to enforce consistency becomes the origin of fragmentation, and the accessibility debt is inherited by every product at once — so when it is finally caught, remediation is not one project but all of them. The discount was taken once, at the foundation. It is paid by every floor above it, indefinitely.

To retention. The requirement that solved sixty percent of the user's problem did not fail. It underperformed, quietly. Users adapt, work around it, or leave. The cost surfaces in churn, a metric nobody traces back to a requirement written three years earlier.

To future velocity. The MVP that was explicitly temporary becomes permanent, because revisiting it means deprioritizing something else. Every team that inherits it pays comprehension tax on the shortcuts and the undocumented decisions. A one-time saving in an estimate compounds into years of slower delivery elsewhere.

To compliance and security. A feature ships without full security modelling because it will be handled next cycle. Audit finds it. What could have been part of the original build becomes urgent, unplanned, expensive rework. The cost was not avoided; it was deferred and amplified.

The pattern is consistent. The cost of the discount is real, it is large, it is distributed, and it is structurally divorced from the decision that caused it. The team that took the discount records a success. The functions that absorb it record a burden with no identifiable origin. The organization pays the bill in full and never sees the invoice.

This is what makes the private price list more than an estimation error. It is a misallocation of accountability. And it means the honest comparison was never "expensive requirement versus cheap requirement." It was "cost paid once, visibly, by the people equipped to argue about it" versus "cost paid forever, invisibly, by everyone else."

Which reframes the diagnostic problem usefully. You cannot audit a private price list directly — it leaves no artifact. But you can audit its consequences. Support volume clustered around a single workflow, an operations team whose headcount is justified entirely by exception handling, a mandated internal tool everybody complains about, a design system with a shadow library of local overrides: these are not unrelated operational irritations. They are receipts.
...
Manage the expectation gap deliberately. If the first conversation routinely produces asks that the second can fund at only twenty percent, contributor fatigue is inevitable. The remedy is not to abandon the separation; it is to make the gap itself a first-class object of attention. Record the full-price requirement, record the staged

version, and treat the difference as an explicit organizational signal — a capability gap, a funding gap, or a prioritization decision. Over time, patterns in that gap become the most honest diagnostic the organization has.
...
The designer who contributes half is not failing at his job — he is doing it well by the standards of a reality that is being dismantled. Reading the team, adapting the artifact, pricing the ask against reality: these remain the disciplines of senior practice. What has changed is what reality costs.
...
You have the mechanism now. The private price list, the biases that corrupt it, the canon that graded the corruption as craft, the two forms the discount takes, and the places the cost quietly lands. You also have the thought experiment, which is the only part of this paper that works on you rather than on your organization.

So the useful thing to say at the end is not another exhortation. It is that once you can see the habit, you cannot easily unsee it — and that is the whole point. You will start noticing it. In the requirement marked down before it reached the backlog. In the "we don't have capacity" stated as fact by someone quoting a test nobody has run since 2023. In the mandated tool everyone works around. In the epic nobody wrote because the enabler spend felt unjustifiable against a ceiling nobody has priced.

Go chase the suboptimal.

Not because your organization is failing. It is probably excellent — at execution, at quality, at moving quickly inside its constraints. That is exactly why the discount survived: it never looked like a defect, it looked like competence. But the constraints have moved, and the price list has not.

Start narrow. One conversation held in two halves. One requirement recorded at full price alongside the version you can actually fund. One ceiling tested instead of inherited. One support queue traced back to the decision that created it.

You know where the discounts live now. Go and collect.
...
- When was the last time a full-price (unconstrained) requirement was explicitly recorded, even if later
staged?

- How often do feasibility judgments cite data older than 12–18 months?
- Are "we tried that" or "the team can't" statements treated as hypotheses to be re-tested, or as settled
facts?

- Does discovery produce at least one undreamed-of requirement as a required output?
- Is the gap between stated need and delivered scope made visible to someone with authority to close it?
- Are exploration and ceiling-testing efforts budgeted separately from delivery?
- Do contributors report that writing ambitious requirements carries social or political cost?
- How frequently are requirements written in outcome language versus implementation language?
- When AI or new tooling has changed a cost assumption, how quickly has the private price list been
updated?

- Is there an explicit organizational signal that full-price asking is protected?
- Can you trace any current support burden, operational workaround, or internal-tool complaint back to a
requirement that was discounted at write time?

A cluster of low scores on 1, 3, 4, 5, and 7 usually indicates need-discounting. Low scores on 2, 6, and 9 usually indicate discovery-discounting. A low score on 11 rarely means the transfer is not happening; it usually means nobody is looking.

## Open Questions

A paper that proposes a new lens earns trust by naming what it has not yet answered.

Evidence is the principal gap. The cost-transfer argument runs on illustrative cases, not measured ones. The cheapest available demonstration is retrospective rather than longitudinal: one organization willing to trace an existing support queue, an operations exception-handling load, or the daily time cost of a mandated internal tool back to the requirement decision that produced it. The data already sits in their ticketing and time-reporting systems. Nobody has connected it to the ask.

A second vignette, outside product design. The discount is shown being made once, by a UX designer. Readers in compliance, finance and enterprise operations currently have to translate. They should not have to.

Terminology triage. Eight named concepts is more than a reader will carry. The half-ask, the private price list, and the distinction between need- and discovery-discounting are load-bearing; the rest should read as description rather than coinage.

Validation by people with standing to disagree. The useful question is not whether practitioners like the framework but whether they can break it. CTOs, heads of product, enterprise architects, principal engineers, transformation leads and experienced business analysts are the readers whose objections would teach the most.

Whether the practices belong in a separate paper. Restoring Unpriced Discovery is closer to a methodology than to an argument, and may deserve its own document rather than a section.

## Further Reading

The literature on requirements, delivery methods, and team capacity was written before the cost curve described here began to invert. The frameworks remain useful as diagnostics of where the habits came from.

For what comes next, watch the emerging practice of organizations that are actively testing ceilings they previously assumed fixed.
...
This paper was produced by the model described in a companion discussion paper, The Bottleneck Has Moved: one accountable human and an agentic team of frontier models handling drafting, critique and revision.

### The authoring team

- Jesper Karlsson — accountable human; product ownership, structure, and every final call
- Fable 5 and Opus 5 (Anthropic) — drafting, structural editing, revision
- GPT 4.5 (OpenAI) — independent critique and adversarial review
- Grok 4.5 (xAI) — independent critique and adversarial review
- Gemini 3.6 (Google) — fact-checking, handling of uncertainty and counter-evidence
Five iterations, and approximately twenty-two individual feedback loops.

The byline carries one name, and the credit list carries six. That is deliberate, and it is the companion paper's argument applied to this one: agents can execute, and they can critique well enough to change an author's mind, but they cannot be accountable. Every claim in this paper is answerable to the human named on the front page.

The verification layer earned its place. Independent agent reviews caught the weakest claims in the early versions — a central term that quietly contradicted the paper's own argument, a closing section that spent three paragraphs explaining its own punchline, and a structural framing that would have been misread as

excluding engineers from discovery. The accountable human overruled the agents more than once, including on the title and on a reviewer's recommendation to reinstate language that had already been rejected on substance.

The paper's own thesis applies to its production. Cheap drafting collapsed the cost of finding out what the argument was. It did not reduce the cost of deciding what was true.

## About This Paper

Disagreement is the point. If part of this is wrong — and some of it will be — the author would rather hear it than not.

Cite as Karlsson, J. (2026). The Half-Ask: Stop Discounting the Incredible. Otrobonita AI Labs. otrobonita.com/whitepapers. DOI: 10.5281/zenodo.21703599

Licence Creative Commons Attribution 4.0 (CC BY 4.0). Share it, quote it, argue with it — attribution is all that is asked.

Contact otrobonita.com · otrobonita.com/whitepapers · jesper@otrobonita.com

The Half-Ask · Otrobonita AI Labs · otrobonita.com/whitepapers
