...
## The Literature That Institutionalized the Conditions
...
**To future velocity.** The MVP that was explicitly temporary becomes permanent, because revisiting it means deprioritising something else. Every team that inherits it pays comprehension tax on the shortcuts and the undocumented decisions. A one-time saving in an estimate compounds into years of slower delivery elsewhere.
...
Which reframes the diagnostic problem usefully. You cannot audit a private price list directly — it leaves no artefact. But you can audit its consequences. Support volume clustered around a single workflow, an operations team whose headcount is justified entirely by exception handling, a mandated internal tool everybody complains about, a design system with a shadow library of local overrides: these are not unrelated operational irritations. They are receipts.
...
You have the mechanism now. The private price list, the biases that corrupt it, the canon that graded the corruption as craft, the two forms the discount takes, and the places the cost quietly lands. You also have the thought experiment, which is the only part of this paper that works on you rather than on your organisation.
...
Not because your organisation is failing. It is probably excellent — at execution, at quality, at moving quickly inside its constraints. That is exactly why the discount survived: it never looked like a defect, it looked like competence. But the constraints have moved, and the price list has not.
...
11. Can you trace any current support burden, operational workaround, or internal-tool complaint back to a requirement that was discounted at write time?
...
*Version 4. Open questions for further iteration: (1) Evidence remains the principal gap — the cost-transfer section argues from illustrative cases, not measured ones. A single organisation willing to trace support volume or internal-tool time cost back to a specific discounted requirement would be worth more than any further prose. (2) A diagram contrasting the discounted path with the unpriced-discovery path, and showing where the transferred cost lands. (3) Validation with practitioners who do not already agree — the questions worth asking are "where does this break?" and "have you seen this happen?", not "do you like it?". (4) Whether the practices section should become a standalone methodology paper. (5) Positioning — published whitepaper, conference keynote, or upstream practice supplement for SAFe and product communities.*
