...
### Why some hallucinations are created long before generation
...
These are not necessarily four isolated conveyor-belt steps. Structuring is the objective that often determines how extraction and cleaning must be performed. In difficult corpora, **extraction and structuring may be inseparable**: the system has to recognise layout while extracting it, because flattening first can destroy relationships that cannot reliably be reconstructed afterward. If a PDF's three columns encode `timestamp | speaker | utterance`, an extraction method that returns all the words but destroys those relationships has not succeeded merely because the text is legible.
...
| Printed page | OCR output | Possible downstream impact |
| --- | --- | --- |
| `00 02 12 04` | `00 02 12 0h` | Exact timestamp lookup or structured GET filtering can fail |
| `00 02 12 43` | `00 02 12 h3` | Timestamp can no longer be reliably parsed as time |
| `00 02 13 14` | `00 02 13 1h` | Ordering or time-based metadata derived from the OCR can be corrupted |
...
| Topology | Boundary that must survive | If lost | Preprocessing response |
| --- | --- | --- | --- |
| **Lateral — scope** | Department, region, product variant, customer tier, system, process family | Compatible claims can appear contradictory | Preserve scope as metadata or an explicit knowledge boundary |
| **Temporal — version** | Effective date, revision, supersession | Obsolete and current claims can compete in retrieval | Preserve revision, effective date, and supersession relationships |
...
This is the sense in which **human slop** enters the system. It does not necessarily mean poor human knowledge or careless source material. The Apollo transcript is neither. It means information designed around human perception and convention becoming ambiguous when treated as though its structure were already machine-readable.
...
This paper was developed through a human-led, multi-model review process: one accountable author working with frontier models for drafting, critique, challenge, and revision.

### The authoring and review team

- **Jesper Karlsson** — accountable human; thesis, product ownership, structure, source selection, and every final call
- **Claude (Anthropic)** — drafting, structural editing, and revision
- **ChatGPT (OpenAI)** — independent critique, synthesis, and revision
- **Grok (xAI)** — independent critique and adversarial review
- **Gemini (Google)** — independent critique, technical challenge, and review

The byline carries one name. The credit list carries the team. That distinction is deliberate: models can draft, challenge, and materially improve an argument, but accountability for what is published remains with the human named on the front page.

The review process earned its place. Early critique challenged claims that treated hallucination too deterministically, forcing a distinction between **grounding errors** and **generation errors**. Reviewers also caught overly universal claims about contradiction and information loss, and pushed a supporting idea — **information topology** — into the conceptual centre of the paper. Later rounds concentrated on making those ideas legible to a stakeholder audience without flattening the technical argument.

The accountable human overruled model recommendations where they were too absolute or implementation-dependent. In particular, proposed claims about OCR errors inevitably causing vector-search failure were narrowed to consequences the evidence actually supports.

The paper's own thesis applies to its production: downstream quality depends heavily on what survives upstream. Cheap drafting made iteration fast. It did not remove the work of deciding what the argument meant, what the evidence supported, or what was true.

## About This Paper

Disagreement is welcome. If part of this argument is wrong — and some of it will be — the author would rather hear it than not.

**Cite as:** Karlsson, J. (2026). *Human Slop In, AI Slop Out: Why some hallucinations are created long before generation.* Otrobonita AI Labs.

**Publication:** otrobonita.com/whitepapers
